/*Challenge link:https://www.codewars.com/kata/5616868c81a0f281e500005c/train/csharp
Question:
...
public class Rank{
    public static string NthRank(string st, int[] we, int n){
      ...
      //check validation
      if(st == null || st == "") return  "No participants";
      if(we.Length < n) return "Not enough participants";
      
      //find score of each name, name is the current element, i is the index
      //convert character to ascii value and mod 32 to get its rank, get the total sum + name's length 
      //then times it by weight, store in array.
      var score = st.Split(",")
                    .Select((name, i) => new {
                      Name = name, 
                      WinNum = (name.Length + name.Sum(ch => ch % 32)) * we[i]})
                    .ToArray();
      
      //order by descending, then by its names, select the last person's name.
      return score.OrderBy(x => -x.WinNum)
                  .ThenBy(x => x.Name)
                  .ElementAt(n - 1).Name;
    }
}
