/*Challenge link:https://www.codewars.com/kata/5263c6999e0f40dee200059d/train/csharp
Question:
...
public class Kata{
  ...
  //include adjacent numbers plus the number itself.
  private static IEnumerable<string>[] keypad = new[]{
      new[] {"0", "8"},
      new[] {"1", "2", "4"},
      new[] {"2", "1", "3", "5"},
      new[] {"3", "2", "6"},
    ...
    public static List<string> GetPINs(string observed){
      ...
      //Aggregate Pins and next, from the selected keypad from string observed
      //using string interpolation to format the string into  {PIN}{next}
      //store in list and return the result.
      return observed.Select(c => keypad[c - '0'])
                     .Aggregate((PINs, next) => 
                     PINs.SelectMany(PIN => next.Select(next => $"{PIN}{next}")))
                     .ToList();;
    }
}
...
public class Tests
{
    ...
    public void TestRandom()
    {
        ...
        {
            string observed = string.Join("", Enumerable.Repeat('0', 5).Select(c => (char)rnd.Next('0', '9' + 1)));
            var submittedResult = Kata.GetPINs(observed);
            var referenceResult = ReferenceSolution.GetPINs(observed);
            submittedResult.Sort();
            referenceResult.Sort();
            CollectionAssert.AreEqual(referenceResult, submittedResult);
        }
    }

    #region reference solution
    private class ReferenceSolution
    {
        public static List<string> GetPINs(string observed)
        {
            if (string.IsNullOrEmpty(observed)) return new List<string>();
            ...
            var result = new List<string>(dict[observed[0]].Select(x => x.ToString()));
            for (int i = 1; i < observed.Length; i++)
            {
                var cur = dict[observed[i]];
                var tmp = new List<string>(result);
                result.Clear();
                for (int j = 0; j < cur.Length; j++)
                    for (int k = 0; k < tmp.Count; k++)
                        result.Add($"{tmp[k]}{cur[j]}");
            }
            return result;
        }
    }
